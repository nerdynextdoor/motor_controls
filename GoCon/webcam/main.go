...
var porting serial.Port
var canRotate = true
var lastAngle = 0
var lastQrScan string
...
func recognizeFile(img image.Image) {
	...
	for _, qrCode := range qrCodes {
		...
		lastQrScan = string(qrCode.Payload)
	}
	//elapsed := time.Since(start)
...
func angleTest(x1, y1, x2, y2 float64) {
	...
	if (lastAngle < (angle + 2) || lastAngle > (angle - 2)) {
		
		if (canRotate == true) {
			fmt.Printf("QR Rotation: %v degrees\n", angle)
			lastAngle = angle
			if (angle > 0) {
				n, err := porting.Write([]byte("_,64,r,2,1,"+strconv.Itoa(angle)))
				if err != nil {
					log.Fatal(err)
				...
				n, err := porting.Write([]byte("_,64,r,1,1,"+strconv.Itoa(angle*-1)))
				if err != nil {
					log.Fatal(err)
					fmt.Printf("this error")
				}
...
func moveBack_Forward() {
	if (lastQrScan == "887" ) {
		//move forward
		n, err := porting.Write([]byte("_,64,m,1,1000,0"))
				if err != nil {
					log.Fatal(err)
					fmt.Printf("this error")
				}
		...
		//move Backwards
		n, err := porting.Write([]byte("_,64,m,2,1000,0"))
				if err != nil {
					log.Fatal(err)
					fmt.Printf("this error")
				}
...
func readData(port serial.Port)  {
	...
	for {
		...
		if (strings.Contains(string(buff[:n]), "okok")) {
			moveBack_Forward();
			canRotate = true;
			fmt.Println(canRotate)
